...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРезультатОбработки

&НаКлиенте
Процедура РезультатОбработкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтрРезультат = Элементы.РезультатОбработки.ТекущиеДанные;
	
	Если Поле.Имя = "РезультатОбработкиОбъект" Тогда
		ПоказатьЗначение(, СтрРезультат.Объект);
	КонецЕсли;
	
	Если Поле.Имя = "РезультатОбработкиПредставлениеРезультата" И Не СтрРезультат.Обработан Тогда
		ОбработкаОшибок.ПоказатьИнформациюОбОшибке(СтрРезультат.ИнформацияОбОшибке,
...
Процедура ИзменитьРеквизитыОбъектов(Команда)
	...
	ПараметрыИзмененияРеквизитов.ОбменДаннымиЗагрузка = ОтключитьБизнесЛогикуПриЗаписи;
	ПараметрыИзмененияРеквизитов.ВыполнятьВТранзакции = ВыполнятьВТранзакции;
	ПараметрыИзмененияРеквизитов.ПрерыватьПриОшибке = ПрерыватьПриОшибке;
	ПараметрыИзмененияРеквизитов.ЗаписьВыполняетсяАлгоритмом = ЗаписьВыполняетсяАлгоритмом;
	...
	Обработано = 0;
	РазмерПорции = ?(ВыполнятьВТранзакции, ВсеСсылки.Количество(), 5);
	ПорцииСсылок = ПорционироватьКоллекцию(ВсеСсылки, РазмерПорции);
	Для Каждого ПорцияСсылок Из ПорцииСсылок Цикл
		...
		ПараметрыИзмененияРеквизитов.Ссылки = ПорцияСсылок;
		
		РезультатыОбработки = ИзменитьРеквизитыОбъектовНаСервере(ПараметрыИзмененияРеквизитов);
		Для Каждого Результат Из РезультатыОбработки Цикл
			СтрРезультат = РезультатОбработки.Добавить();
			СтрРезультат.Объект = Результат.Ссылка;
			СтрРезультат.Обработан = Результат.Обработан;
			СтрРезультат.ИнформацияОбОшибке = Результат.ИнформацияОбОшибке;
			
			Если СтрРезультат.Обработан Тогда
				СтрРезультат.ПредставлениеРезультата = НСтр("ru='Обработан успешно'");
			Иначе
				Шаблон = НСтр("ru='Ошибка: %1'");
				ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(СтрРезультат.ИнформацияОбОшибке);
				СтрРезультат.ПредставлениеРезультата = СтрШаблон(Шаблон, ТекстОшибки);
			КонецЕсли;
		КонецЦикла;
		
		ЕстьОшибки = (РезультатОбработки.НайтиСтроки(Новый Структура("Обработан", Ложь)).Количество() > 0);
		Если ЕстьОшибки И ПрерыватьПриОшибке Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Состояние("Изменение завершено");
	Элементы.СтраницыЭтапы.ТекущаяСтраница = Элементы.СтраницаЭтапРезультатВыполнения;
	НастроитьКнопкиКоманднойПанели();
КонецПроцедуры
...
Процедура НастроитьКнопкиКоманднойПанели()
	...
	ВыбранаСтраницаРезультат = (Элементы.СтраницыЭтапы.ТекущаяСтраница = Элементы.СтраницаЭтапРезультатВыполнения);
	...
	Элементы.ФормаГруппаРезультатВыполнения.Видимость = ВыбранаСтраницаРезультат;
	...
	ИначеЕсли ВыбранаСтраницаРезультат Тогда
		Элементы.ФормаВернутьсяКВыборуОбъектовИзРезультата.КнопкаПоУмолчанию = Истина;
	Иначе
...
Функция ИзменитьРеквизитыОбъектовНаСервере(Знач Параметры)
	...
	Для Каждого Ссылка Из Параметры.Ссылки Цикл
		...
		Для Каждого СтрЗначенияРеквизитов Из Параметры.ИзменяемыеРеквизиты Цикл
		...
		Попытка
			...
			Если Параметры.ОбменДаннымиЗагрузка Тогда
