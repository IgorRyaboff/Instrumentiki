...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыЗапроса") Тогда
		...
		ПриИзмененииПараметровЗапроса(ЭтотОбъект);
		Модифицированность = Истина;
	КонецЕсли;
	
	МодульОбъекта = РеквизитФормыВЗначение("Объект");
	ОграничениеТипа = МодульОбъекта.ТипЛюбаяСсылка(Истина);
	ОграничениеТипа = Новый ОписаниеТипов(Новый ОписаниеТипов("Число, Строка, Булево, Дата, УникальныйИдентификатор"),
		ОграничениеТипа.Типы());
	//
	Элементы.ПараметрыЗапросаЗначение.ОграничениеТипа = ОграничениеТипа;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Для Сч = 0 По ПараметрыЗапроса.Количество() - 1 Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.Имя) Тогда // Сработает стандартная проверка
			Продолжить;
		КонецЕсли;
		
		Попытка
			Структура = Новый Структура;
			Структура.Вставить(Стр.Имя);
		Исключение
...
Процедура ЗаполнитьПараметрыИзЗапроса(Команда)
	...
	Для Каждого КлючЗнч Из СтруктураПараметров Цикл
		...
		Если ТипЗнч(ТипЗначенияИлиТаблицаЗначений) = Тип("ОписаниеТипов") Тогда
			...
			СтрПараметры.Значение = ТипЗначенияИлиТаблицаЗначений.ПривестиЗначение(Неопределено);
...
Асинх Процедура СгенерироватьТекстДляКонфигуратора(Команда)
	...
	ТекстВопроса = "Сформировать также шаблон программного кода (создание объекта Запрос и цикл выборки)?";
	Ответ = Ждать ВопросАсинх(ТекстВопроса, Кнопки);
	Если Ответ = КодВозвратаДиалога.Отмена Тогда
		Возврат;
	КонецЕсли;
	ФормироватьПрограммныйКод = Ответ;
...
Процедура УМ_Сохранение_ПриЧтенииОбъектаПереопределяемый(Пакет, ВерсияФормата)
	...
	Для Каждого СтрХДТО Из Пакет.ПараметрыЗапроса Цикл
		...
		СтрПриемник.Значение = СтрХДТО.Значение.Получить();
	КонецЦикла;
...
Асинх Процедура ВыбратьВидПараметраАсинх(СтрПараметрыЗапроса)
	...
	ИначеЕсли ВидПараметра = "Таблица" Тогда
		СтрПараметрыЗапроса.Значение = НовоеХранилищеЗначенияСТаблицейЗначений();
...
&НаСервереБезКонтекста
Функция НовоеХранилищеЗначенияСТаблицейЗначений()
	Возврат Новый ХранилищеЗначения(Новый ТаблицаЗначений);
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Процедура ПриИзмененииПараметровЗапроса(Форма)
	Для Каждого СтрПараметры Из Форма.ПараметрыЗапроса Цикл
		Если ТипЗнч(СтрПараметры.Значение) = Тип("ХранилищеЗначения") Тогда
			СтрПараметры.ПредставлениеТипа = НСтр("ru='Таблица значений'");
		Иначе
			СтрПараметры.ПредставлениеТипа = ТипЗнч(СтрПараметры.Значение);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
