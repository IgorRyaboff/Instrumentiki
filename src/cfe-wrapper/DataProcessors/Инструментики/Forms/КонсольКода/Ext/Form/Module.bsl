...
Процедура ВыполнитьКод(Команда)
	...
	Иначе
		...
		Если ТипЗнч(РезультатВыполнения.ОбменДанными) <> Тип("Соответствие")
			И ТипЗнч(РезультатВыполнения.ОбменДанными) <> Тип("Структура") Тогда
			
			Шаблон = "Объект ОбменДанными в коде был заменен на значение недопустимого типа (%1).
				|Таблица ""Обмен данными"" была очищена";
			ПоказатьПредупреждение(, СтрШаблон(Шаблон, ТипЗнч(РезультатВыполнения.ОбменДанными)));
			
		Иначе
			Для Каждого КЗ Из РезультатВыполнения.ОбменДанными Цикл
				СтрОбменДанными = ОбменДанными.Добавить();
...
Процедура УМ_Сохранение_ПриЧтенииОбъектаПереопределяемый(Пакет, ВерсияФормата)
	Текст = Пакет.Текст;
	ПривилегированныйРежим = Пакет.ПривилегированныйРежим И Не БезопасныйРежим();
