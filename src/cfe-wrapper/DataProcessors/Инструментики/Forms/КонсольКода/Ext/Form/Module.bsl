...
Процедура ВыполнитьКодНаСервереБезКонтекста(Знач консолькодаКод, Знач консолькодаПривилегированныйРежим, Знач консолькодаКоличествоИтераций, ОбменДанными)
	...
	Для консолькодаСчетчик = 1 По консолькодаКоличествоИтераций Цикл
		...
		Исключение
			ВызватьИсключение("При выполнении кода произошла ошибка",,
				"ИнструментикиОшибкаВыполненияКода",, ИнформацияОбОшибке());
		КонецПопытки;
	КонецЦикла;
КонецПроцедуры
...
Процедура ВызватьВыполнениеКода(консолькодаКоличествоИтераций)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		Если ИнформацияОбОшибке().Код = "ИнструментикиОшибкаВыполненияКода" Тогда
			ОбработкаОшибок.ПоказатьИнформациюОбОшибке(ИнформацияОбОшибке().Причина,
