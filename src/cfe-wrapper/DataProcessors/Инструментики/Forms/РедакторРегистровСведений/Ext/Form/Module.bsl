...
Асинх Процедура ПерейтиКНаборуЗаписей(Команда)
	...
	ОписанияРежимовЗамещения = Новый Массив;
	// BSLLS:LineLength-off
	// Неделимые строки для вывода пользователю
	ОписанияРежимовЗамещения.Добавить(НСтр("ru = '""Замещение"":
	...
	ОписанияРежимовЗамещения.Добавить(НСтр("ru = '""Добавление"":
	...
	Если Объект.ВерсияПлатформы >= 3.25 Тогда
		Кнопки.Добавить(ПредопределенноеЗначение("РежимЗамещения.Слияние"));
		Кнопки.Добавить(ПредопределенноеЗначение("РежимЗамещения.Удаление"));
		
		// BSLLS:LineLength-off
		// Неделимые строки для вывода пользователю
		ОписанияРежимовЗамещения.Добавить(НСтр("ru = '""Слияние"":
		...
		ОписанияРежимовЗамещения.Добавить(НСтр("ru = '""Удаление"":
                                          |Из регистра сведений будут удалены все записи, ключи которых совпадают с ключами записей, которые находятся в записываемом наборе записей. Значения ресурсов и реквизитов не имеют значения и могут принимать любые значения.'"));
		// BSLLS:LineLength-on
	КонецЕсли;
	
	// Добавляем кнопку "Отмена" только после условного добавления других кнопок
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	
	Шаблон = НСтр("ru='Выберите вариант работы с набором записей регистра.
		|
		|%1'");
	ТекстВопроса = СтрШаблон(Шаблон, СтрСоединить(ОписанияРежимовЗамещения, Символы.ПС + Символы.ПС));
	Ответ = Ждать ВопросАсинх(ТекстВопроса, Кнопки,, КодВозвратаДиалога.Отмена);
