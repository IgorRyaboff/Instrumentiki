...
Функция ДанныеДляИзмененияСуществующейИБ(Знач ПараметрыСоединенияСКластером, Знач ПараметрыСоединенияСИБ,
	...
	Если ПараметрыСоединенияСКластером.МетодПодключения = "COM" Тогда
		...
		Если ВерсияПлатформы >= 3.25 Тогда
			...
			ЗадержкаВыгрузки = ИБ.ConfigurationUnloadDelayByWorkingProcessWithoutActiveUsers;
			ОписаниеИБ.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом = ЗадержкаВыгрузки;
		КонецЕсли;
	...
	ИначеЕсли ПараметрыСоединенияСКластером.МетодПодключения = "RAS" Тогда
		...
		Если ВерсияПлатформы >= 3.25 Тогда
			МаксимальныйСдвигРегламентныхЗаданий = ИБ.МаксимальныйСдвигЗапускаРегламентныхЗаданийБезАктивныхПользователей;
			ОписаниеИБ.МаксимальныйСдвигЗапускаРегламентныхЗаданий = МаксимальныйСдвигРегламентныхЗаданий;
			
			МинимальныйПериодРегламентныхЗаданий = ИБ.МинимальныйПериодЗапускаРегламентныхЗаданийБезАктивныхПользователей;
			ОписаниеИБ.МинимальныйПериодЗапускаРегламентныхЗаданий = МинимальныйПериодРегламентныхЗаданий;
			
			ЗадержкаВыгрузки = ИБ.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессомБезАктивныхПользователей;
			ОписаниеИБ.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом = ЗадержкаВыгрузки;
		КонецЕсли;
...
Процедура ЗаписатьИнформационнуюБазуCOM(СоединениеСКластером, ОписаниеИнформационнойБазы,
	...
	Если ВерсияПлатформы >= 3.25 Тогда
		ЗадержкаВыгрузки = ОписаниеИнформационнойБазы.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом;
		ИБ.ConfigurationUnloadDelayByWorkingProcessWithoutActiveUsers = ЗадержкаВыгрузки;
		
		МаксимальныйСдвиг = ОписаниеИнформационнойБазы.МаксимальныйСдвигЗапускаРегламентныхЗаданий;
		ИБ.MaxScheduledJobsStartShiftWithoutActiveUsers = МаксимальныйСдвиг;
		
		МинимальныйПериод = ОписаниеИнформационнойБазы.МинимальныйПериодЗапускаРегламентныхЗаданий;
		ИБ.MinScheduledJobsStartPeriodWithoutActiveUsers = МинимальныйПериод
	КонецЕсли;
...
Процедура ЗаписатьИнформационнуюБазуRAS(СоединениеСКластером, ОписаниеИнформационнойБазы,
	...
	Если ВерсияПлатформы >= 3.25 Тогда
		...
		МаксимальныйСдвиг = ОписаниеИнформационнойБазы.МаксимальныйСдвигЗапускаРегламентныхЗаданий;
		ИБ.МаксимальныйСдвигЗапускаРегламентныхЗаданийБезАктивныхПользователей = МаксимальныйСдвиг;
		
		МинимальныйПериод = ОписаниеИнформационнойБазы.МинимальныйПериодЗапускаРегламентныхЗаданий;
		ИБ.МинимальныйПериодЗапускаРегламентныхЗаданийБезАктивныхПользователей = МинимальныйПериод;
	КонецЕсли;
...
Процедура НачатьАвторизациюВКластере()
	...
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.АдминистрированиеСервера_АвторизацияВКластере",
