...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ЗапретитьЛокальноеРаспознаваниеРечи.Видимость = (Объект.ВерсияПлатформы >= 3.24);
	Элементы.ГруппаПриОтсутствииАктивныхПользователей.Видимость = (Объект.ВерсияПлатформы >= 3.25);
КонецПроцедуры
...
Функция ИменаСвойствДляЗаполненияПараметровСоединенияИБ()
	Возврат "Имя, Описание, ЗащищенноеСоединение, БлокировкаНачалаСеансов,
		|БлокировкаРегламентныхЗаданий, ОбязательноеВнешнееУправление, РазрешитьВыдачуЛицензийСервером,
		|РезервированиеРабочихПроцессов, ВнешнееУправлениеСеансами, ПрофильБезопасности,
		|ПрофильБезопасностиБезопасногоРежима, ТипСУБД, ИмяБазыСУБД, ЛогинСУБД, ПарольСУБД, СерверСУБД,
		|НачалоБлокировкиСеансов, ОкончаниеБлокировкиСеансов, КодРазрешения, ПараметрБлокировкиСеансов,
		|СообщениеБлокировкиСеансов, КодЯзыка, СоздатьБазуДанных, СмещениеДат,
		|ЗапретитьЛокальноеРаспознаваниеРечи, МаксимальныйСдвигЗапускаРегламентныхЗаданий,
		|МинимальныйПериодЗапускаРегламентныхЗаданий, ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом";
КонецФункции
