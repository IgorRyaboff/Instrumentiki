...
Процедура СписокИнструментовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ТекущиеДанные = Элементы.СписокИнструментов.ТекущиеДанные;
	...
	СтандартнаяОбработка = Ложь;
	ОткрытьФормуИнструмента(ТекущиеДанные.ИмяФормы);
КонецПроцедуры
...
&НаКлиенте
Асинх Процедура ОткрытьФормуИнструмента(Имя)
	ИмяОткрываемойФормы = "ВнешняяОбработка.Инструментики.Форма." + Имя;
	
	ВыборОкна = Новый СписокЗначений;
	ВыборОкна.Добавить(Неопределено, НСтр("ru='<новое окно>'"));
	
	Для Каждого ОткрытоеОкно Из ПолучитьОкна() Цикл
		Для Каждого ФормаОкна Из ОткрытоеОкно.Содержимое Цикл
			Если ТипЗнч(ФормаОкна) = Тип("ФормаКлиентскогоПриложения") И ФормаОкна.ИмяФормы = ИмяОткрываемойФормы Тогда
				ВыборОкна.Добавить(ФормаОкна, ФормаОкна.Заголовок);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	ВыбраннаяФорма = Неопределено;
	Если ВыборОкна.Количество() > 1 Тогда
		Ответ = Ждать ВыборОкна.ВыбратьЭлементАсинх(НСтр("ru='Выберите окно'"));
		Если Ответ = Неопределено Тогда
			Возврат;
		КонецЕсли;
		ВыбраннаяФорма = Ответ.Значение;
	КонецЕсли;
	
	Если ВыбраннаяФорма <> Неопределено Тогда
		ВыбраннаяФорма.Активизировать();
	Иначе
		ОткрытьФорму(ИмяОткрываемойФормы,,, Строка(Новый УникальныйИдентификатор));
	КонецЕсли;
КонецПроцедуры
