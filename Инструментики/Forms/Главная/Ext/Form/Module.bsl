...
Процедура ОткрытьКаналTelegram(Команда)
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общая_ВнешняяСсылка", Новый Структура("Ссылка", "https://t.me/instrumentiki1C"));
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьИнфостарт(Команда)
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общая_ВнешняяСсылка", Новый Структура("Ссылка", "https://infostart.ru/1c/tools/1946049"));
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьБагтрекер(Команда)
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общая_ВнешняяСсылка", Новый Структура("Ссылка", "https://redmine.iryaboff.ru/projects/instrumentiki"));
КонецПроцедуры
...
Асинх Процедура ПроверитьОбновленияАсинх()
	...
	Попытка
		Соединение = Новый HTTPСоединение("publicfiles.iryaboff.ru", 443,,,, 5, SSL);
		Запрос = Новый HTTPЗапрос("instrumentiki_updates/latest_stable");
	...
	НомерВерсии = HTTPОтвет.ПолучитьТелоКакСтроку();
	Если НомерВерсии = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.Версия) И СравнитьВерсии(Объект.Версия, НомерВерсии) = 1 Тогда
		...
		Если Объект.НеНапоминатьОбОбновленииДо < ТекущаяДата() И ЗначениеЗаполнено(Объект.Версия) Тогда
			Кнопки = Новый СписокЗначений;
