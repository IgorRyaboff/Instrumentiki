...
Процедура СгенерироватьКод(Команда)
	...
	Если ВидТелаЗапроса = "БезТела" Тогда
		ОсновнойШаблон = СтрЗаменить(ОсновнойШаблон, "[УстановкаТелаЗапроса]", "");
	ИначеЕсли ВидТелаЗапроса = "Строка" Тогда
		Текст = "HTTPЗапрос.УстановитьТелоИзСтроки(""""); // TODO: Вставить содержимое";
		ОсновнойШаблон = СтрЗаменить(ОсновнойШаблон, "[УстановкаТелаЗапроса]", Текст);
	ИначеЕсли ВидТелаЗапроса = "ДвоичныеДанные" Тогда
	...
	СтрокиШаблона = СтрРазделить(ОсновнойШаблон, Символы.ПС, Истина);
	...
	ОсновнойШаблон = СтрСоединить(СтрокиШаблона, Символы.ПС);
