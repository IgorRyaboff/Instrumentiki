...
Процедура СгенерироватьКод(Команда)
	...
	Для Каждого Стр Из ЗаголовкиЗапроса Цикл
		...
		ТекстЗаголовки = ТекстЗаголовки + Символы.ПС + СтрШаблон("HTTPЗапрос.Заголовки[""%1""] = ""%2"";", Ключ, Значение);
		Если Не ПустаяСтрока(Стр.Описание) Тогда
			ТекстЗаголовки = ТекстЗаголовки + " // " + СокрЛП(Стр.Описание);
		КонецЕсли;
	КонецЦикла;
	
	ОсновнойШаблон = СтрЗаменить(ОсновнойШаблон, "[Заголовки]", ТекстЗаголовки);
...
Функция ВыполнитьHTTPЗапросНаКлиентеНаСервере(СтруктураЗапроса, НакопленноеВремяВыполнения = 0)
	...
	Если СтруктураЗапроса.ЗащищенноеСоединение Тогда
		...
		ЗащищенноеСоединение.СертификатыУдостоверяющихЦентров = Новый СертификатыУдостоверяющихЦентровWindows;
	КонецЕсли;
	
	НачалоЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Соединение = Новый HTTPСоединение(СтруктураЗапроса.ИмяСервера, СтруктураЗапроса.Порт, СтруктураЗапроса.ИмяПользователя, СтруктураЗапроса.Пароль,, СтруктураЗапроса.Таймаут, ЗащищенноеСоединение, СтруктураЗапроса.МетодАутентификации = "ОС");
