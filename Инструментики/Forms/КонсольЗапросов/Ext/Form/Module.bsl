...
Процедура ПодзаголовокФормыПриИзменении(Элемент)
	Если ЗначениеЗаполнено(ПодзаголовокФормы) Тогда
		Заголовок = СтрШаблон(НСтр("ru='%1: Консоль запросов'"), ПодзаголовокФормы);
	Иначе
		Заголовок = НСтр("ru='Консоль запросов'");
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура КрасивыйРедакторДокументСформирован(Элемент)
	Вью = ВьюКрасивогоРедактора(Элемент);
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Вью.init(СистемнаяИнформация.ВерсияПриложения);
	
	Вью.setLanguageMode(?(Элемент = Элементы.ТекстЗапросаКрасивыйРедактор, "bsl_query", "bsl")); // См. https://vk.cc/cs5MzW
	Вью.updateMetadata(СтруктураМетаданныхДляКрасивогоРедактора());
	
	Вью.setContent(ЭтотОбъект[Элемент.Подсказка]);
	
	Вью.setOption("generateModificationEvent", true);
	Вью.setOption("generateOnKeyDownEvent", true);
	Элементы.ПереключитьРедакторКода.Доступность = Истина;
КонецПроцедуры

&НаКлиенте
Процедура КрасивыйРедакторПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если СобытиеРедактора.event = "EVENT_CONTENT_CHANGED" Тогда
		ЭтотОбъект[Элемент.Подсказка] = ВьюКрасивогоРедактора(Элемент).getText();
		Модифицированность = Истина;
...
Процедура ПараметрыЗапросаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТекДанные.ВидПараметра = "Таблица" И Поле = Элементы.ПараметрыЗапросаЗначение Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.КонсольЗапросов_ТаблицаЗначений", ПараметрыФормы,,,,, ОписаниеЗакрытия);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПараметрыЗапросаЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекДанные.ВидПараметра = "Список" Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общая_СписокЗначений", ПараметрыФормы,,,,, ОписаниеЗакрытия);
	КонецЕсли;
КонецПроцедуры
...
Процедура ФактическиУстановленныеПараметрыЗапросаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТипЗнч(СтрПараметры.Значение) = Тип("СписокЗначений") Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общая_СписокЗначений", ПараметрыФормы);
	Иначе
...
Асинх Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	ОбновитьТекстВКрасивомРедактореКода();
...
Процедура ПереключитьРедакторКода(Команда)
	...
	Элементы.ТекстЗапросаПростойРедактор.Видимость = Не ИспользуетсяКрасивыйРедакторКода;
	Элементы.ТекстЗапросаКрасивыйРедактор.Видимость = ИспользуетсяКрасивыйРедакторКода;
	
	Элементы.ПередВыполнениемПростойРедактор.Видимость = Не ИспользуетсяКрасивыйРедакторКода;
	Элементы.ПередВыполнениемКрасивыйРедактор.Видимость = ИспользуетсяКрасивыйРедакторКода;
	
	Элементы.ПриВыборкеПростойРедактор.Видимость = Не ИспользуетсяКрасивыйРедакторКода;
	Элементы.ПриВыборкеКрасивыйРедактор.Видимость = ИспользуетсяКрасивыйРедакторКода;
	
	Элементы.ПослеВыполненияПростойРедактор.Видимость = Не ИспользуетсяКрасивыйРедакторКода;
	Элементы.ПослеВыполненияКрасивыйРедактор.Видимость = ИспользуетсяКрасивыйРедакторКода;
	
	Элементы.ПереключитьРедакторКода.Доступность = Не ИспользуетсяКрасивыйРедакторКода; // Включится по событию редактора ДокументСформирован
	Элементы.ПереключитьРедакторКода.Пометка = ИспользуетсяКрасивыйРедакторКода;
КонецПроцедуры
...
Асинх Процедура ЗаполнитьИзНабораДанныхСКД(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общее_ОткрытиеСКД",,,,,, ОповещениеОЗакрытии);
КонецПроцедуры
...
Процедура ПрочитатьУниверсальныйПакет(Адрес)
	...
	ИмяОбъекта = "";
	ПриОпределенииНастроекИнструментаXDTO(Неопределено, ИмяОбъекта, Неопределено, Неопределено);
...
Асинх Функция СохранитьАсинх()
	...
	Попытка
		...
		#КонецЕсли
	Исключение
		Ждать ПредупреждениеАсинх(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()),, "Ошибка сохранения данных");
	КонецПопытки;
...
&НаКлиенте
Функция ВьюКрасивогоРедактора(Элемент)
	Возврат ?(Элемент.Документ = Неопределено, Неопределено, Элемент.Документ.defaultView);
КонецФункции

&НаКлиенте
Процедура ОбновитьТекстВКрасивомРедактореКода()
	Вью = ВьюКрасивогоРедактора(Элементы.ТекстЗапросаКрасивыйРедактор);
	Если Вью <> Неопределено Тогда
		Вью.setContent(ТекстЗапроса);
	КонецЕсли;
	
	Вью = ВьюКрасивогоРедактора(Элементы.ПередВыполнениемКрасивыйРедактор);
	Если Вью <> Неопределено Тогда
		Вью.setContent(ПередВыполнением);
	КонецЕсли;
	
	Вью = ВьюКрасивогоРедактора(Элементы.ПриВыборкеКрасивыйРедактор);
	Если Вью <> Неопределено Тогда
		Вью.setContent(ПриВыборке);
	КонецЕсли;
	
	Вью = ВьюКрасивогоРедактора(Элементы.ПослеВыполненияКрасивыйРедактор);
	Если Вью <> Неопределено Тогда
		Вью.setContent(ПослеВыполнения);
	КонецЕсли;
КонецПроцедуры
