...
Процедура ПодзаголовокФормыПриИзменении(Элемент)
	Если ЗначениеЗаполнено(ПодзаголовокФормы) Тогда
		Заголовок = СтрШаблон(НСтр("ru='%1: Консоль СКД'"), ПодзаголовокФормы);
	Иначе
		Заголовок = НСтр("ru='Консоль СКД'");
	КонецЕсли;
КонецПроцедуры
...
Асинх Процедура ЗагрузитьСхему(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.Общее_ОткрытиеСКД", ПараметрыФормы,,,,, ОповещениеОЗакрытии);
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьНастройки(Команда)
	ПараметрыФормы = ПараметрыФормыПользовательскихНастроек();
	Форма = ПолучитьФорму("ВнешняяОбработка.Инструментики.Форма.КонсольСКД_Настройки", ПараметрыФормы);
	...
	#Иначе
		...
		Иначе
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.ЗапускСеанса");
			КонецЕсли;
		КонецЕсли;
	#КонецЕсли
КонецПроцедуры
...
Процедура ОткрытьКонсольЗапросов(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.Инструментики.Форма.КонсольЗапросов", ПараметрыФормы);
КонецПроцедуры
