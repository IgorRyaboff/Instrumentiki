...
Процедура ПодзаголовокФормыПриИзменении(Элемент)
	Если ЗначениеЗаполнено(ПодзаголовокФормы) Тогда
		Заголовок = СтрШаблон(НСтр("ru='%1: Удаление недопустимых символов XML в объекте'"), ПодзаголовокФормы);
	Иначе
		Заголовок = НСтр("ru='Удаление недопустимых символов XML в объекте'");
	КонецЕсли;
КонецПроцедуры
